...
/// App configuration. The backend base URL is read from Info.plist (GOEMON_API_BASE)
/// so it can differ per build configuration; falls back to the local dev backend.
enum Config {
    static var apiBase: String {
        (Bundle.main.object(forInfoDictionaryKey: "GOEMON_API_BASE") as? String) ?? "http://localhost:3001"
    }
