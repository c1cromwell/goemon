...
test.describe("Agent · SmartChat", () => {
  ...
  test("a balance question gets a reply", async ({ page }) => {
    ...
    await page.getByPlaceholder("Message Goemon Global Finance…").fill("what's my balance?");
  ...
  test("a transfer over $500 triggers the MFA gate", async ({ page }) => {
    await page.getByPlaceholder("Message Goemon Global Finance…").fill("send $600 to blair@demo.com");
    await page.getByRole("button", { name: "Send" }).click();
