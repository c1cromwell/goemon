/**
 * One-time account linking — the demo stand-in for what the Goeman Global Finance portal + iOS
...
const LINK_STORE = "goeman_agent_link";
...
export async function linkAccount(email: string, password: string, scopes: string[]): Promise<LinkState> {
  ...
  await api.grant(token, {
    agentDid: CLIENT_DID,
    displayName: "Goeman Global Finance Assistant (demo agent)",
