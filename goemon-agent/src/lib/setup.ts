/**
 * One-time account linking — the demo stand-in for what the Goemon Global Finance portal + iOS
...
const LINK_STORE = "goemon_agent_link";
...
export async function linkAccount(email: string, password: string, scopes: string[]): Promise<LinkState> {
  ...
  await api.grant(token, {
    agentDid: CLIENT_DID,
    displayName: "Goemon Global Finance Assistant (demo agent)",
