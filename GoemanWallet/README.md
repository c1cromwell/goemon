...
```
GoemanWallet/
  GoemanWalletApp.swift        TabView (Setup · Credential · Wallet · Activity) + deep links
...
1. New Xcode project → iOS App → SwiftUI, name `GoemanWallet`, bundle id
   `com.goemanglobal.wallet`. Delete the generated `ContentView.swift` and the
   auto-generated Info.plist.
2. Add all files under `GoemanWallet/` to the target (keep the folder groups).
3. Build settings: set **INFOPLIST_FILE** to `GoemanWallet/Info.plist` and turn
...
4. **File → Add Package Dependencies…** → `https://github.com/hiero-ledger/hiero-sdk-swift`
   (from `0.49.0`) → add product **Hiero** to the GoemanWallet target. Required for
