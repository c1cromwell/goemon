...
`EQUITY_ISSUER`) decouples Goemon from the backing/redemption mechanism:
...
- **v1 — distribute a regulated 1:1 issuer (recommended first).** Integrate an issuer that already does
  ...
  - Goemon = on-chain wallet + compliance + distribution + dividend/redemption **pass-through**. Lightest
    path; the issuer holds the shares, is the transfer agent, and bears the securities-law weight.
- **v2 — first-party issuance (later phase).** Goemon custodies real shares via a partner and mints its
