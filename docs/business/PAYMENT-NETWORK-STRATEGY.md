# Payment Network Strategy — can Goemon build a card on a *new* (non-Visa/MC/Amex) rail?
...
> legal or investment advice. It reconciles with — and extends — `docs/goemon_prdv1/06-payments-and-rails.md`
> (which currently plans the card as a **Visa/MC debit, v2**), `CORPORATE-STRUCTURE.md` (the Corp A/B/C ramp),
> and `docs/GOEMON-PLAN.md` (Phases 19–20).
...
- **A new *rail* (stablecoin / programmable) for a focused wedge:** **genuinely credible, and the only
  honest path to "not Visa/MC/Amex."** You own issuance (the Goemon wallet/users) and acceptance (merchants
  ...
  agent-authorized payments** — which Goemon already has the primitives for.
...
near-impossible head-on) is the trap. Goemon can have a **brand on Visa now** and a **rail of its own** for a
...
## 4. The reframe — a stablecoin-settled, agent-native rail (where Goemon is actually strong)

The credible "new network" doesn't replace Visa's acceptance; it builds a **different rail** where Goemon
*already owns both sides* and the incumbents are weak.

**What "the network" means here:**
- **Issuance side:** the Goemon non-custodial wallet + user base (you control it).
...
**The genuine unlock legacy networks cannot match — programmable, scoped, agent-authorized payments.** Goemon
...
2. **Online / API checkout** — a "Pay with Goemon (USDC)" button; integration is software, not hardware.
...
4. **Agent-to-merchant / machine commerce** — the structurally-defensible frontier (§4); AI agents paying
   under scoped grants. Few competitors, and Goemon is ahead on the primitives.
...
- **Start lighter, in order:**
  ...
  2. **Stablecoin-/RWA-collateralized credit** — already designed as **PRD v2 collateralized lending**
     (`06-payments-and-rails.md` §Lending): pledge Goemon-held RWA tokens, borrow USDC, auto-liquidate on LTV
...
- **Card brand / issuing** → extends **Phase 19 — Full-bank rails** in `docs/GOEMON-PLAN.md` (BaaS partner +
  MSB already the gate there); the Visa-bridge card is a Phase-19 deliverable.
- **Native stablecoin settlement rail + agent-native payments** → **Phase 21 — "Goemon Pay"** (Corp B/C),
  ...
  **Stage-1 prototype now BUILT** (see `docs/GOEMON-PLAN.md` Phase 21): merchants + payment intents,
  escrow-protected pay→capture/refund/dispute, the `pay_merchant` MCP scope for agent-to-merchant commerce,
  zero rail fee, `GOEMON_PAY_ENABLED` kill-switch (prod-fatal until licensed).
- **Credit** → **collateralized** lending is PRD v2 (rides the rail); **unsecured** is PRD v3 (lending
  license/partner + capital).

> **One-line summary for the cap table / a pitch:** *Goemon isn't building "a cheaper Visa" — that's a
