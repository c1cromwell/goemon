...
A review of the Goemon Fraud Engine architecture and how **Google CEL** (Common Expression Language,
...
- **Money-path determinism.** `rules-v1` stays **prod**; CEL ships **shadow-first** and never changes the
  live decision until someone promotes it. The engine is standalone (Goemon calls it over HTTP), so there
  is zero Goemon money-path risk.
