...
`argus_prdv1/01`), and it's the **lowest-securities-risk** real-world asset (a whole graded card is a
...
| 6 | **Argus Pay (stablecoin rail)** | Corp B/C · `ARGUS_PAY_ENABLED` ⚖ | "Pay any merchant, agent-native." | Power users → payments |
...
§8 (positioning), `argus_prdv1/01`,`/05` (vision + marketplace). Product surface: `sellerCollectibleService`,
