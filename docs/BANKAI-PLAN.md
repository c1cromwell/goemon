# BankAI — Master Plan (consolidated, Hedera-aligned, production-hardened)
...
>
> **Strategic context:** this evolves the BankAI prototype toward the architecture in the Bankai PRD (Hedera settlement, native wallet, double-entry ledger). The credential + agent-authorization layer is the foundation and is kept intact; the simulated-banking core is replaced with a correct ledger and a Hedera integration seam.
...
- **Token form (NFT vs fungible) & what actually moves the custody line.** A recurring question is
  whether holding the asset as an **NFT with smart contracts handling value/ownership** changes BankAI's
  ...
  *characterization* — custody turns on **who controls the keys and the funds**, not on the token standard.
  ...
  - **Custody turns on key/fund control — three separate things:**
    ...
    - *Asset/token custody* — reduced **only** by the locked **non-custodial** model (keys in the Secure
      Enclave, server never holds the private key). If the user holds the token, BankAI isn't the asset
      custodian; if BankAI holds the keys, it is — NFT or not.
  ...
  - **The real lever, and its catch.** A **trust-minimized contract** holding the deed-NFT + escrow, where
    BankAI holds **no admin/upgrade/freeze keys**, supports a "the protocol is the custodian" argument.
...
8.5   Direct asset transfer (user-to-user)
      ...
      Non-securities transfer freely between BankAI users. Reuses the transferService shape
...
8.12  Phase 8 E2E validation gate
      ...
      external-agent paths driven via the bankai-mcp-test-harness skill.
```

**Out of scope (PRD-05/10 production/v2 items, noted not built):** first-party Bankai token issuance;
...
## Phase 15 — Internal Agent Operations (DESIGN ONLY)

**Status: designed, not built.** This phase runs the bank's *back office* — support, KYC review,
...
16.2  Agent/MCP-driven coverage — the journeys that are genuinely NL- or client-driven:
      ...
      Driven via the bankai-mcp-test-harness skill acting as a real client.
...
16.3  Skills (built under .claude/skills/):
      ...
      - bankai-mcp-test-harness — thin MCP/HTTP client exercising SmartChat, the operation-token exchange,
...
- **Tokenized RWA / collectibles marketplace** — now planned as **Phase 8** above (prototype-buildable
  scope). The production/v2 items (first-party issuance, deployed ERC-3643 + audit, bridging/CCTP, AMM,
  auctions, real partner integrations) remain out of scope — see the Phase 8 out-of-scope note.
- **Temporal / Conductor orchestration** — the prototype calls the Anthropic SDK directly, which is fine at this scale.
  The internal agent-operations design (governance, security controls, compliance automation, and the
  prototype→Conductor/Temporal migration seam) is **Phase 15** above; the engine migration itself is Phase 15.4.
