...
**Status: DESIGN (not built).** A role-gated internal web app — an **Goemon Command Center** — that gives
...
**Related:** `docs/GOEMON-PLAN.md` (master plan) · `docs/AGENTIC-OS.md` (CEO approvals + agent fleet) ·
...
Goemon today has **many RBAC-gated admin APIs** and **Prometheus metrics**, but almost no **unified operator UI**:
...
| `fraud-engine` `:4500` | Case queue, decisions, remediation | Separate service; no SOC dashboard in Goemon |
...
| **Goemon Pay** | Payment intents paid 30d; GMV; open disputes | `payment_intents`, `escrow_events` |
...
| **Goemon Brain** | Route “show me company health” → CEO pulse |
