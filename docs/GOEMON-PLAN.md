# Goemon Global Finance — Master Plan (consolidated, Hedera-aligned, production-hardened)
...
>
> **Strategic context:** this evolves the Goemon Global Finance prototype toward the architecture in the Goemon Global Finance PRD (Hedera settlement, native wallet, double-entry ledger). The credential + agent-authorization layer is the foundation and is kept intact; the simulated-banking core is replaced with a correct ledger and a Hedera integration seam.
...
│
├── goemon-agent/                            # (unchanged from v1)
│   └── ...
│
└── GoemonWallet/
    └── GoemonWallet/
        ├── GoemonWalletApp.swift
        ├── ContentView.swift
        ├── Views/
        │   ├── SetupView.swift
...
```
Establish the cross-cutting conventions for the Goemon Global Finance rebuild. These rules apply to every later phase.
...
```
Create a Node.js + Express + TypeScript backend for Goemon Global Finance.
...
- Create /backend/src/db/index.ts:
  - getDb() returns a thin query wrapper. If DATABASE_URL is set → Postgres (pg Pool). Else → better-sqlite3 at
    ./data/goemon.db with WAL + foreign_keys ON.
...
```
Add a Hedera integration seam so Goemon Global Finance accounts map to real Hedera testnet accounts and USDC. This is the foundation
...
- **Token form (NFT vs fungible) & what actually moves the custody line.** A recurring question is
  whether holding the asset as an **NFT with smart contracts handling value/ownership** changes Goemon Global Finance's
  ...
  *characterization* — custody turns on **who controls the keys and the funds**, not on the token standard.
  ...
  - **Custody turns on key/fund control — three separate things:**
    ...
    - *Asset/token custody* — reduced **only** by the locked **non-custodial** model (keys in the Secure
      Enclave, server never holds the private key). If the user holds the token, Goemon Global Finance isn't the asset
      custodian; if Goemon Global Finance holds the keys, it is — NFT or not.
  ...
  - **The real lever, and its catch.** A **trust-minimized contract** holding the deed-NFT + escrow, where
    Goemon Global Finance holds **no admin/upgrade/freeze keys**, supports a "the protocol is the custodian" argument.
...
8.5   Direct asset transfer (user-to-user)
      ...
      Non-securities transfer freely between Goemon Global Finance users. Reuses the transferService shape
...
8.12  Phase 8 E2E validation gate
      ...
      external-agent paths driven via the goemon-mcp-test-harness skill.
```

**Out of scope (PRD-05/10 production/v2 items, noted not built):** first-party Goemon Global Finance token issuance;
...
GoemonWalletApp.swift: TabView with Setup, Credential, Wallet, Activity. Keep onOpenURL for argus-wallet:// (consent)
...
```
Build goemon-agent exactly as in v1 (intentDetector, mcpClient, walletBridge, MessageBubble, TokenIndicator,
...
3. Start services: backend (npm run dev), frontend (5173), goemon-agent (5174),
   iOS Wallet (Xcode, iPhone 15 sim).

4. Seed admin (RBAC): POST /api/admin/seed → admin@goemonglobal.com / Admin1234! with role 'admin'.

5. Register simulator agent as MCP client (admin JWT):
POST /api/admin/mcp-clients
{ "client_did":"did:simulator:agent-app", "display_name":"Goemon Global Finance Simulator Agent",
...
16.2  Agent/MCP-driven coverage — the journeys that are genuinely NL- or client-driven:
      ...
      Driven via the goemon-mcp-test-harness skill acting as a real client.
...
16.3  Skills (built under .claude/skills/):
      ...
      - goemon-mcp-test-harness — thin MCP/HTTP client exercising SmartChat, the operation-token exchange,
...
- **Issuance — both, as phases:** a swappable `EquityIssuer` seam (`EQUITY_ISSUER`); **v1** distributes a
  regulated 1:1 issuer (**Dinari** US / **Backed**, **Ondo** non-US) with Goemon as wallet + compliance +
  distribution + dividend/redemption pass-through; **v2** first-party issuance (Goemon + transfer agent +
...
Add the "money app" rails that make Goemon a daily-driver account — **without** becoming a bank: partner
...
- **Scope:** fiat **on/off-ramp** (19.1), **FBO/escrow** accounts for held balances (19.2 — never the
  ...
  partner*, never marketed as our own — see the "bank naming" rule the Goemon rebrand serves).
...
- **Fraud Stages 2–4:** **BUILT as a standalone add-on** (`fraud-engine/`, Node/TS :4500) that graduates
  ...
  **routing + shadow/canary** (`models/`, `router/`), an append-only decision topic, an analyst **case
  queue** (`cases/`), an async **remediation** loop that calls Goemon back to **freeze/flag** (`remediation/`),
  and a **label → retrain** loop (`learning/`). The service imports **nothing** from `backend/`. **Hybrid
  integration:** an in-Goemon triage (`fraudService`) routes each money event blocking (sync `fraudClient`
...
## Phase 22 — Goemon Starter (13+ family/teen wealth-building suite) → **Corp B/C** (DESIGN)
...
- **Solution:** Extend `frontend/` `/admin/command/*` — **Goemon Command Center** — backed by a new
...
- **Solution:** Nine workstreams (24.1–24.9) with staged engineering + partner steps; x401/x402 as HTTP layers
  atop existing OID4VP/VC + Goemon Pay (no bypass of VP verification or ledger idempotency).
...
cd argus/goemon-agent && npm run dev
# Xcode: open GoemonWallet/GoemonWallet.xcodeproj, run on iPhone 15 simulator
...
|---|---|---|
| Admin | admin@goemonglobal.com | Admin1234! (RBAC role: admin) |
