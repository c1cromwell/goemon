...
Goemon Global Finance uses a progressive identity model where capability unlocks happen at each tier rather than at a single signup gate. Users tier up only when they hit a feature that requires it.
...
Adds phone number (SMS OTP verified) and email (if not provided at Tier 0). Phone serves as one factor of recovery and as a uniqueness check (one Goemon Global Finance account per verified phone, soft-enforced).
