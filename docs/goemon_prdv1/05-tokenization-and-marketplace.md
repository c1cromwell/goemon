...
Goemon Global Finance does not issue tokens in v1. We list third-party tokenized assets through partnerships. Target ~10 assets across three categories at launch:
...
For v1, the listings are *displayed and orderable* through Goemon Global Finance but settlement may happen on the issuer's native chain with Goemon Global Finance acting as an order router. Where the asset is available natively on Hedera (BENJI is targeting this; BUIDL is exploring), settlement is local.
...
Goemon Global Finance uses ERC-3643 for any tokenized asset that is or could be a security. The standard enforces transfer restrictions on-chain via:
...
- **KYC key** — Goemon Global Finance-controlled key that can grant/revoke KYC status per holder
- **Freeze key** — Goemon Global Finance-controlled key that can freeze individual holders (for fraud, sanctions, etc.)
- **Wipe key** — used rarely, for fraud or court-ordered remediation
- **Custom fees** — optional royalty to issuer/Goemon Global Finance on transfer

**Requirements:**
- `[REQ-MK-TOK-005]` HTS tokens for collectibles include a Freeze key controlled by Goemon Global Finance's compliance multisig
...
- `[REQ-MK-LIFE-002]` Listings can be paused or delisted by Goemon Global Finance at any time; user holdings are preserved but new orders are blocked
...
- Real estate fractions: order book matching internal buy/sell intent across Goemon Global Finance users where the asset's transfer rules permit

### Direct transfer (user-to-user)

For non-securities assets, direct transfer between Goemon Global Finance users is supported. ERC-3643 securities require recipient to be on the Identity Registry; same-user transfer to an external wallet is possible if the destination address is registered.
...
Goemon Global Finance's revenue model on the marketplace is a combination of:
...
- `[REQ-MK-FEE-003]` Fee structure is uniform per asset class; no tier-based fee discounts in v1 (added in v2 with Goemon Global Finance Plus)

## Out of scope for v1

- First-party tokenization (Goemon Global Finance issuing its own RWA tokens) — v2
- Derivatives, leverage, margin trading
- Tokenized equities (legally complex in US; international only in v2)
- Tokenized debt obligations of Goemon Global Finance users (peer lending)
