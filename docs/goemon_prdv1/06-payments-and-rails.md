...
US users (Tier 2+) can:
- **Deposit** by pushing ACH from their external bank to Goemon Global Finance (one-time or recurring)
- **Withdraw** by initiating ACH from Goemon Global Finance to their linked external bank
...
The simplest payment surface — Goemon Global Finance user to Goemon Global Finance user, or Goemon Global Finance user to any external Hedera address.

**Requirements:**
- `[REQ-PAY-P2P-001]` Goemon Global Finance-to-Goemon Global Finance transfers settle in under 5 seconds end-to-end (Hedera finality + our notification latency)
- `[REQ-PAY-P2P-002]` Goemon Global Finance-to-external transfers settle at Hedera finality (~3 seconds)
...
User pledges Goemon Global Finance-held RWA tokens (BUIDL, OUSG, etc.) as collateral; borrows USDC against them. Mechanism:
- Smart contract on HSCS locks the collateral
- USDC is disbursed to user from a Goemon Global Finance liquidity pool
...
This is a securities-collateralized loan and routes through a regulated entity (US: through partner bank or registered broker-dealer; international: through Goemon Global Finance's local entity per jurisdiction).
...
> **Network strategy note:** the v1/v2 card rides Visa/MC (below). Whether Goemon could instead build a
...
- Real-time authorization through Goemon Global Finance's processor (Marqeta, Lithic, or partner bank's processor)
...
| USDC send (Goemon Global Finance→Goemon Global Finance) | 4s | 10s | Hedera consensus + our DB |
| USDC send (Goemon Global Finance→external) | 4s | 10s | Hedera consensus |
