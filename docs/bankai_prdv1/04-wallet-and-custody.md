...
Bankai is **non-custodial** for all on-chain assets (USDC, USDT, RWA tokens, NFTs). The user's Hedera account holds their assets; Bankai operates the infrastructure that lets them use that account through the app.

For US dollars (real bank-rail dollars, distinct from USDC), Bankai is **custodial-via-partner-bank** at Tier 2+. The partner bank holds the USD in a pooled or FBO ("for benefit of") account; user balances are tracked in our ledger and reconciled with the bank daily. Tier 0 and Tier 1 users do not have USD balances — only USDC.
...
```
User device                              Bankai backend                   Hedera network
─────────────                            ──────────────                   ──────────────
┌────────────────────┐                  ┌──────────────────┐              ┌────────────┐
│  Bankai app        │                  │  Wallet Service  │              │   Hedera   │
...
- Decryption requires the user's passkey signature, which Bankai cannot produce on the user's behalf
...
**Requirements:**
- `[REQ-WALLET-006]` Each Bankai user account has exactly one Hedera account ID at any time
- `[REQ-WALLET-007]` New users' Hedera accounts are created on first sign-in with a single device-derived key; account creation cost is sponsored by Bankai's paymaster
...
- Any token issued by Bankai's first-party tokenization (v2+)
...
**Requirements:**
- `[REQ-WALLET-012]` USDC association happens automatically at account creation; cost (currently ~$0.05 of HBAR equivalent) is sponsored by Bankai
...
Users never see HBAR. Bankai pays all transaction fees on the user's behalf using Hedera's transaction fee delegation model.

**Mechanism:**
- Bankai operates a "paymaster" account funded with HBAR
...
**Requirements:**
- `[REQ-WALLET-015]` Gas sponsorship covers: token transfers up to a daily quota per user, marketplace purchases, smart contract calls to whitelisted Bankai contracts
...
- **Wallet export to third parties from within Bankai by default.** Users can export their Hedera private key via a clearly-labeled "Advanced" flow that includes a security warning, step-up auth, and a 24-hour cooldown, but we don't make it a primary action. Export is for users leaving the platform.
...
- Hardware wallet integration (Ledger, Trezor) for the primary Bankai wallet
