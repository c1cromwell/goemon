...
Bankai ships three client surfaces and one internal surface at launch:
...
| Internal admin console | Repurposed Bankai CLI (Python) | Used by compliance reviewers, customer support escalations, ops |
...
2. User can copy address, share via system share sheet, or generate a payment request link (deep link that opens to a pre-filled send screen if recipient also has Bankai)
...
1. Move → "Send" → enter recipient (Bankai handle, Hedera address, EVM address, or contact)
2. Enter amount
...
- `[REQ-SEND-003]` Sending to a known Bankai handle short-circuits address entry and shows the recipient's name/avatar for confirmation
...
- Subscription tiers (Bankai Plus, Pro, etc.) — single free tier at launch
