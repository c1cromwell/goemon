...
import { getGoemonClient } from "../remediation/goemonClient";
...
export function buildRoutes(ctx: Context): ExpressRouter {
  ...
  // Analyst action on a case: freeze/unfreeze the user (calls Goemon back).
  r.post(
    "/v1/cases/:id/action",
    asyncH(async (req, res) => {
      const c = await ctx.cases.get(req.params.id!);
      ...
      if (action === "freeze") {
        ...
        await getGoemonClient().freeze({ userId: c.userId, reason: req.body?.reason ?? "analyst freeze", decisionId: c.decisionId ?? c.id });
      } else if (action === "unfreeze") {
        await ctx.cases.recordAction(c.id, "unfreeze_requested", actor, req.body?.reason);
        await getGoemonClient().unfreeze({ userId: c.userId, reason: req.body?.reason ?? "analyst unfreeze", decisionId: c.decisionId ?? c.id });
