 ...
 * (mirrors the Goemon backend's posture) — the service-auth key must not be the
...
const schema = z.object({
  ...
  GOEMON_BASE_URL: z.string().url().default("http://localhost:3001"),
  GOEMON_SERVICE_KEY: z.string().min(1).default(KNOWN_DEV_API_KEY),
...
export const config = {
  ...parsed,
  GOEMON_BASE_URL:
    parsed.GOEMON_BASE_URL ||
    (process.env.ARGUS_BASE_URL as string | undefined) ||
    "http://localhost:3001",
  GOEMON_SERVICE_KEY:
    parsed.GOEMON_SERVICE_KEY ||
