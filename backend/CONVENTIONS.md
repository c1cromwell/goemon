...
- `audit_logs`, `ledger_entries`, `ledger_journals`, `mcp_audit_logs`, `fraud_decisions`, `fills`,
  `escrow_events` are **append-only** (DB triggers block UPDATE/DELETE). Never attempt to mutate them.
