...
describe("Phase 5A: admin RBAC", () => {
  it("seed + authenticate; wrong password is rejected", async () => {
    ...
    expect(seeded.email).toBe("admin@goemonglobal.com");

    const ok = await authenticateAdmin("admin@goemonglobal.com", "Admin1234!");
    expect(ok.role).toBe("admin");

    await expect(authenticateAdmin("admin@goemonglobal.com", "wrong")).rejects.toThrow();
  });
