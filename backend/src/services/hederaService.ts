...
/**
 * Transfer USDC on-chain from a Goemon Global Finance user to any Hedera account address,
 * then post a matching ledger journal so the internal book balance stays in sync.
 *
 * @param input.toUserId  If set, the recipient is a Goemon Global Finance user: credit their
...
/** Look up a Goemon Global Finance user's Hedera account ID. Throws NOT_FOUND if they have none. */
