 ...
 *
 * Merchants (directly-integrated counterparties owned by an Goemon user) request
 ...
 * GOEMON_PAY_ENABLED kill-switch gates NEW intents and payments only — capture,
...
function assertPayEnabled(): void {
  if (!config.GOEMON_PAY_ENABLED) {
    throw new AppError(ErrorCode.PAY_DISABLED, "Goemon Pay is currently unavailable");
  }
}
...
export async function payIntent(input: {
  ...
  const escrow: EscrowRow = await hold({
    ...
    memo: `Goemon Pay: ${raw.merchant_name} (${raw.id})`,
