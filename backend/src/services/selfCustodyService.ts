 ...
 * freezing/suspending accounts. This makes Argus's non-custodial guarantee TANGIBLE
 ...
 * the user — or anyone — can verify Argus's statement independently. No money moves.
 */
...
export async function getSelfCustodyReport(userId: string): Promise<SelfCustodyReport> {
  ...
  return {
    ...
    frozen,
    guarantee: [
      "Argus never holds your wallet's signing key — it lives on your device (Secure Enclave). No one at Argus can sign or move funds on your behalf.",
      "Your on-chain / self-custodied assets are yours and cannot be frozen or seized by Argus.",
 ...
 * needs to control their assets independently of Argus. Proves there is no lock-in.
 */
export async function getExportManifest(userId: string): Promise<{ manifest: ExportManifest; signedManifestJwt: string }> {
  ...
  const manifest: ExportManifest = {
    ...
    holdings: portfolio,
    instructions: [
      "Your wallet signing key is on your device (Secure Enclave) — back it up there; Argus cannot recover it for you (by design).",
      "Your Hedera account id + public key let you view and control on-chain assets via any Hedera-compatible wallet/explorer.",
      "This manifest is signed by Argus's issuer key and verifiable against /.well-known/jwks.json.",
    ],
